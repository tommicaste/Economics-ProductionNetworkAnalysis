...
%     end
    %tau_ij=ones(N,N);
    %tau_ij(1:54,55)=0.5;
    %xij_hat=xij_hat.*tau_ij;
    xij_hat([1:54,58:end],56)=0.2;
...
%sector price = marginal cost
    if theta == 1
        RES1 = log(pi_hat) - ( -log(zi_hat) + eta.*log(vi_hat) + (1-eta).*log( Pmi_hat ) );
    else
        RES1 = pi_hat_1MoinsThetai -  (zi_hat).^(theta-1).*( eta.* vi_hat.^(1-theta) + (1-eta).* Pmi_hat.^(1-theta)  );
    end
